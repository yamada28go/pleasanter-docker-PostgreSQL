...
- `AWS_ACCESS_KEY_ID` と `AWS_SECRET_ACCESS_KEY` が環境変数として渡っていること
- `AWS_DEFAULT_REGION` または `AWS_REGION` が環境変数として渡っていること
- `S3_TARGET_BUCKET_NAME` と `S3_TARGET_DIRECTORY_NAME` が環境変数として渡っていること
...
1. `.env.secrets` に `ENABLE_S3_BACKUP=1` を設定する
2. `.env.secrets` に `AWS_ACCESS_KEY_ID` と `AWS_SECRET_ACCESS_KEY` を設定する
3. `.env.secrets` に `AWS_DEFAULT_REGION` または `AWS_REGION` を設定する
4. `.env.secrets` に `S3_TARGET_BUCKET_NAME` と `S3_TARGET_DIRECTORY_NAME` を設定する
...
S3_TARGET_BUCKET_NAME=my-backup-bucket
...
- S3 設定が有効な場合は、実行後に `sync_to_s3.sh dumpall false` により `dumpall` ディレクトリが S3 同期されます
...
- `.env.secrets`
  ...
  - `AWS_DEFAULT_REGION` または `AWS_REGION`
